		...
		try {
			...
			if (type === 'compaction/start') {
				if (cfg.compactionGuard) {
					writeGuard(tc, 'start', { turn: typeof data.turn === 'number' ? data.turn : null, compactionId: stringOrEmpty(data.compactionId) });
				}
				if (cfg.autoSnapshot) {
					const turn = eventTurn(data);
			...
			if (type === 'compaction/summary') {
				if (cfg.compactionGuard) {
					writeGuard(tc, 'summary', { compactionId: stringOrEmpty(data.compactionId) });
				}
				if (cfg.autoSnapshot) {
