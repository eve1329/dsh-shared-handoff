...
function resolveTaskContext(session, cfg, cache) {
	...
	if (autoBound === false && transcriptPath !== undefined) {
		autoBound = bindSessionTask(stateDir, transcriptPath, taskId);
	}
	// First contact with the default task: also point `current-task` at it,
	// like pi's writeCurrentTask on resolution.
	if (taskId === DEFAULT_TASK && !existsSync(join(stateDir, 'current-task'))) {
		try {
