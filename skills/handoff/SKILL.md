...
This is the dsh (DeepSeek Harness) edition of the shared handoff kit. The
state layout it reads and writes under `.agents/state/` is identical to the
Codex and Claude editions, so handoffs can cross agent boundaries: export
here, resume in Codex, and vice versa.
...
Do not invent a parallel state system when one already exists. When this
plugin's `task-id-bootstrap` skill is active in the same repo, the canonical
state files are `.agents/state/tasks/<task-id>/process.md` and
`process.recent.md` — update those instead of inventing new ones.
...
- `.agents/state/tasks/<task-id>/process.md` when the task-id workflow is in use
...
On Windows the workspace path uses drive letters
(`C:\absolute\path\to\repo`); the rest of the format is identical.
